    ...
    <aside class="sidebar">
      ...
      <div class="paciente-profile">
        ...
        <div class="paciente-info">
          <span class="paciente-name">{{ userName }}</span>
          <span class="paciente-role">Paciente</span>
        </div>
      </div>

      <nav class="sidebar-nav">
        <router-link to="/paciente/dashboard" class="nav-item" active-class="active">
        ...
        <router-link to="/paciente/info-personal" class="nav-item" active-class="active">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <router-link to="/paciente/equipo" class="nav-item" active-class="active">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <router-link to="/paciente/agendar" class="nav-item" active-class="active">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <router-link to="/paciente/configuracion" class="nav-item" active-class="active">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
import { authService, clearUser, getUser, pacienteNotificacionesService } from '@/services/api'

const router      = useRouter()
const currentUser = computed(() => getUser())
const userName    = computed(() => currentUser.value?.nombre ?? 'Paciente')
const initials    = computed(() => {
  const n = currentUser.value?.nombre ?? 'P'
...
async function cargarNotificaciones() {
  try {
    const res = await pacienteNotificacionesService.getAll()
...
async function marcarLeida(n) {
  if (n.leida) return
  try {
    await pacienteNotificacionesService.marcarLeida(n.notificacion_id)
    n.leida = true
  } catch {}
}
...
async function marcarTodas() {
  try {
    await pacienteNotificacionesService.marcarTodas()
